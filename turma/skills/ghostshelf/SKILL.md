---
name: ghostshelf
description: Build and run a faceless (no-face) content page that sells a small digital product, end to end. Invoke when someone wants to start a faceless Instagram or TikTok gallery account, create and sell a digital template/guide/spreadsheet on Gumroad, choose a profitable no-face niche, batch carousel/slideshow content, or wire a link-in-bio funnel to an email list, newsletter, or store. Walks the user through niche choice, product creation, content batching, distribution, and the weekly operating loop, with honest odds baked in. Reads the project's brand.md for niche, voice, and the funnel destination.
---
...
It is the honest version. It tells the user the real odds before they spend a weekend, and it optimizes for the one lever that actually decides the outcome: distribution of a specific fix to the people who already pay for it. The numbers below come from the public economics of the faceless-creator model (storefront data, platform reach rules, real seller outcomes), reduced to an operating procedure.

**Before you start:** read the project's `brand.md`. The niche, the voice, and the funnel destination (the primary conversion goal) come from there. If the project is greenfield and `brand.md` is thin or missing, ghostshelf helps pick them: Step 1 chooses the niche, Step 4 sets the voice and look, and the funnel points at whatever `brand.md` names as the primary conversion (default: an email opt-in).

## Read this first: the honest odds

Say this to the user before anything else. Do not skip it.

- The model is real. People have built six and seven figure businesses selling templates with no face attached. Margins are ~95%; you keep ~$17-18 of every $20 on most platforms.
...
If `brand.md` already names the audience and the splinter, use that as the lead candidate and validate it against the four filters below. If the project is greenfield, interview the user: pull out 3-5 things they know how to do that other people pay to have done. For each candidate, score it against four filters. All four must be a clear yes:
...
Output: one chosen problem, written as a single sentence, plus the evidence that people already pay for the fix. Do not move on until this exists. If it diverges from `brand.md`, flag it so the owner can update the brief.
...
- Write the product title and the storefront description: lead with the outcome, name the buyer, list what is inside, set the price.
...
- A signature visual wrapper: one recognizable look the page never breaks. It could be a single illustration style, a fixed duotone with one accent color, a consistent photo treatment, or a repeating texture. Pick ONE and match it to the brand's voice and adjectives in `brand.md`. The specific look matters less than never breaking it.
...
- **Need conversion (saves + opt-ins):** Swipe File ("steal these prompts/templates"), Checklist / Audit, Step-by-Step, Framework, Q&A. The Swipe File is the strongest bridge to an opt-in: preview the assets, trade the full version for a subscribe. (This is a micromagnet; see `turma:micromagnet-craft`.)
...
- Expect a 2-4 month slow ramp before anything accelerates. Judge progress on saves/sends and conversions (from `brand.md`), not follower count. A 500-follower page with high saves beats a 10k page with none.
- If after ~8 weeks of consistent, genuinely useful posting there are no saves and no conversions, the problem is usually the niche (Step 1) or the hook (Step 5), not the algorithm. Re-run Step 1.
...
- Any public copy this skill produces must pass the house rules in `brand.md` (dashes, banned phrases, AI tells) and the universal voice rules. Run `turma:anti-ai-linguo` as the final pass before shipping.
...
- Reads the project's `brand.md` before starting. Niche, voice, look, and funnel destination come from there, never hardcoded.
- Never edits operating canon (`SOUL.md`, `IDENTITY.md`, `CLAUDE.md`, `SPEC.md`, `memory/`).
- Never publishes autonomously. Pages, products, and public copy go to the owner for review before they ship (Yellow tier).
- Never invents facts or fake proof. The product and its results must be real; ask the user for the specifics only they know.
- Never names real people or fabricates testimonials. Early testimonials come from real buyers who reply.
- No AI tells, no dashes. Run `turma:anti-ai-linguo` as the final pass on any public copy.
