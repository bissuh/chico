...
Saves source MP4s to {workdir}/assets/source_hooks/{channel_slug}/.
...
Examples:
    ./scrape_shorts.py https://www.youtube.com/@SomeCreator 20
    ./scrape_shorts.py @SomeCreator 10

Requires YOUTUBE_API_KEY in .env. yt-dlp must be on PATH.
...
def search_shorts(channel_id: str, count: int) -> list[str]:
    ...
    We filter by duration <=90s in fetch_video_details to keep only true Shorts."""
    ids: list[str] = []
    page_token = None
...
def main():
    ...
    for i, video in enumerate(shorts, 1):
        ...
        if path:
            video["local_path"] = str(path.relative_to(WORKDIR))
