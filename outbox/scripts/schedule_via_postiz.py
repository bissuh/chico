...
Reads:
  - outbox/assets/postiz_uploads.json  (uploaded files with their hosted URLs)
  - outbox/assets/source_hooks/{slug}/manifest.json  (source video metadata for sort)
...
Usage:
  ./schedule_via_postiz.py --start 2026-05-10T01:00:00Z --count 50 --interval 6
...
LEDGER_PATH = REPO_ROOT / "outbox" / "assets" / "postiz_uploads.json"
HOOKS_DIR = REPO_ROOT / "outbox" / "assets" / "source_hooks"

DEFAULT_TITLE = "Start a Business with AI | thebillionperson.com"
DEFAULT_DESCRIPTION = (
    "Get the free weekly playbook: thebillionperson.com\n\n"
    "4,000+ readers learning how to start AI-powered businesses with vibe-coding tools."
)
DEFAULT_TAGS = [
    {"value": "AI", "label": "AI"},
    ...
    {"value": "Make Money With AI", "label": "Make Money With AI"},
...
def main():
    ...
    parser.add_argument("--start", default="2026-05-10T01:00:00Z", help="UTC ISO start time")
    ...
    for iso, c in plan:
        ...
        if ok:
            ...
            save_ledger(ledger)
            print(f"  ✓ {iso}  {c['video_id']}  -> {payload}")
        else:
            failures += 1
            print(f"  ✗ {iso}  {c['video_id']}  {payload}")
