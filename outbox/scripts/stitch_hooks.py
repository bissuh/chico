#!/usr/bin/env python3
"""
Stitch the first N seconds of each scraped short onto the back of a CTA video.

Reads outbox/assets/source_hooks/{channel_slug}/manifest.json,
extracts the first HOOK_SEC seconds of each source short,
concatenates the CTA video, normalizes to 1080x1920 9:16,
writes finished MP4s to outbox/assets/finished/{channel_slug}/{video_id}.mp4.
...
Defaults:
    cta_path       = outbox/assets/CTAvideo_v7.mp4
    hook_seconds   = 3

Examples:
    ./stitch_hooks.py alex_hormozi
    ./stitch_hooks.py my_first_million_pod outbox/assets/CTAvideo_v7.mp4 3
"""
...
ASSETS = REPO_ROOT / "outbox" / "assets"
HOOKS_DIR = ASSETS / "source_hooks"
FINISHED_DIR = ASSETS / "finished"

DEFAULT_CTA = ASSETS / "CTAvideo_v7.mp4"
DEFAULT_HOOK_SEC = 3
...
def main():
    ...
    if not cta.exists():
        sys.exit(f"CTA video not found: {cta}")
    ...
    print(f"CTA:     {cta.relative_to(REPO_ROOT)}")
    ...
    print(f"Finished videos: {out_dir.relative_to(REPO_ROOT)}")
