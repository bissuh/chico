...
- **DEPTH STANDARD (non-negotiable).** A carousel is a complete, genuinely useful mini-guide, NOT thin one-liners. Rules:
  - **8-10 slides** for a real teaching post (not 3-4). HARD CAP 10: Instagram's carousel API rejects more than 10 images (a 12-slide post ERRORS on IG; TikTok allows 35). Use the `detail` layout for real paragraphs, the `prompt` layout for actual copy-paste prompts.
