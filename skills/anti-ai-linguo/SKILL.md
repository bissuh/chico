---
name: anti-ai-linguo
description: Final-pass voice check that strips AI tells (banned phrases, dashes, pseudo-contrarian hooks, hedged opinions) from any TBP public draft. Auto-runs as the last review pass before any newsletter, X post, LinkedIn post, IG caption, or video script is marked review-ready. Three modes — long-form (articles), short-form (social), video (scripts). Skip only for internal notes and Saturday newsletter (Bissuh's voice, hands off).
---

# anti-ai-linguo

The final voice-check pass on any TBP public draft. Three modes share the same banned list but apply different structural rules.

**When to run:** as the LAST pass before marking anything review-ready. After story-craft, after micromagnet-craft, after every structural edit. Voice integrity is the last gate, not the first.

**Skip:** internal notes, working sessions, Bissuh's Saturday newsletter (his voice, untouched).

## Mode selection

Pick by format, not by length:

- **Long-form** — articles, newsletter editions (non-Saturday), deep dives, lead-magnet pages. Anything meant to be read silently in a feed reader or browser.
...
If the draft spans formats (e.g. a thread + a blog post version), run the relevant mode for each separately.

---

## Universal rules (apply to all three modes)
...
Every vague claim either gets a number, a name, or a specific moment — or it gets cut.
...
6. The ask (reply, follow, click — ONE CTA).
...
- Frame success around AVD, not views. (See [[feedback_avd_over_views]].)
...
When in doubt, ask: would Bissuh actually say this to a friend at the kitchen table?
...
- [[story-craft]] — runs BEFORE this skill, on persuasive narrative content. anti-ai-linguo is the voice pass after the structural work is done.
- [[micromagnet-craft]] — runs alongside this skill on public content. Different question (is there a magnet hiding inside?) but same final-pass discipline.
- [[name-craft]] — separate use case (naming things). No overlap.

## Public mirror note

A sanitized version of this skill lives in the public chico repo as a lead-magnet handraiser ("stop your AI from sounding like AI"). When updating this file, also update the public version. See [[project_skills_in_chico_repo]].
