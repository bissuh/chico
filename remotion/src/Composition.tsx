...
const KenBurns: React.FC<{
  ...
  panX: number;
  panY: number;
}> = ({src, localFrame, duration, panX, panY}) => {
  ...
  if (!src) {
    return (
      <AbsoluteFill style={{background: `linear-gradient(180deg, ${TEAL} 0%, ${TEAL_DARK} 100%)`}} />
    );
  }
  ...
  return (
    <AbsoluteFill style={{overflow: 'hidden'}}>
      ...
      />
      <AbsoluteFill
        style={{backgroundColor: TEAL, mixBlendMode: 'multiply', opacity: 0.6}}
      />
      <AbsoluteFill
        style={{
          background:
...
export const TBPCta: React.FC<CtaProps> = ({headline1, headline2, subhead, url, bg1, bg2, bg3}) => {
  const frame = useCurrentFrame();
    ...
    <AbsoluteFill style={{backgroundColor: '#000'}}>
      {op1 > 0 && (
        <AbsoluteFill style={{opacity: op1}}>
          <KenBurns src={src1} localFrame={frame} duration={SCENE_1_LEN} panX={-8} panY={-4} />
